# Solution: Level 9 / Project 11 - Recovery Time Estimator
...
# WHY linear interpolation for percentiles? -- With 500 simulation runs,
# the exact 90th percentile may fall between two data points. Linear
# interpolation gives a smoother, more accurate estimate than simply
# rounding to the nearest index.
...
# WHY severity multipliers per phase? -- SEV1 incidents have faster
# detection (alerts fire immediately) but harder diagnosis (pressure,
# complexity). SEV3 incidents have slower detection (no alerts) but
# easier fixes. Multipliers encode this domain knowledge.
...
def default_estimation_strategy(profile: IncidentProfile) -> list[PhaseEstimate]:
    ...
    for phase in RecoveryPhase:
        ...
        # WHY runbooks cut diagnosis/fix time by 40%? -- A runbook provides
        # a pre-written procedure for known failure modes. Engineers skip
        # the "figure out what's wrong" phase and go straight to the fix.
        # The 0.6 multiplier reflects real-world MTTR improvements from
        ...
        # WHY sqrt for team size scaling? -- Adding engineers has diminishing
        # returns (Brooks's Law). 4 engineers are not 4x faster than 1; they're
        # roughly 2x faster (sqrt(4) = 2). The 0.4 floor prevents the estimate
        # from dropping below 40% of the solo time.
...
|----------|-----|----------------------|
| Monte Carlo simulation with triangular distribution | Produces probability distributions (confidence intervals) from three-point estimates; triangular distribution is the simplest model for min/mode/max | Single-point estimate -- gives false precision; stakeholders need to know the range of possible outcomes |
| Phase-based recovery model (detect, diagnose, fix, verify, deploy) | Matches real incident response phases; each phase has different drivers and can be optimized independently | Single-phase estimate -- hides which phase is the bottleneck |
| Frozen dataclass for PhaseEstimate | Phase estimates are computed once and should not be mutated during simulation | Mutable dataclass -- risks accidental mutation during Monte Carlo sampling |
| Sqrt scaling for team size | Models Brooks's Law (diminishing returns from adding engineers); a 4-person team is ~2x faster, not 4x | Linear scaling (divide by team size) -- unrealistically assumes perfect parallelism |
| Strategy pattern for estimation functions | Different organizations can plug in custom estimation strategies with their own base estimates and multipliers | Hardcoded estimation logic -- prevents customization for teams with different incident profiles |

## Alternative approaches

### Approach B: Historical data-driven estimation with PERT distribution

```python
class HistoricalEstimator:
    """Use past incident data to calibrate estimates instead of
    expert-provided three-point estimates."""
    def __init__(self):
        self._history: dict[str, list[float]] = {}  # phase -> list of actual times

    def record_actual(self, phase: str, actual_minutes: float):
        self._history.setdefault(phase, []).append(actual_minutes)

    def estimate_phase(self, phase: str) -> PhaseEstimate:
        """Derive min/mode/max from historical data."""
        data = sorted(self._history.get(phase, []))
        if len(data) < 3:
            return PhaseEstimate(RecoveryPhase(phase), 10, 30, 120)  # fallback
        return PhaseEstimate(
            phase=RecoveryPhase(phase),
            min_minutes=data[0],
            expected_minutes=data[len(data) // 2],  # median
            max_minutes=data[-1],
        )
```

**Trade-off:** Historical data produces more accurate estimates than expert guesses, because past incidents reflect the actual recovery capability of the team. The tradeoff is that you need enough historical data (at least 10-20 incidents per severity level), and the estimates become stale as the team's tooling and processes improve. Use expert estimates when starting out; transition to historical data as your incident database grows.

## Common pitfalls

| Scenario | What happens | Prevention |
|----------|-------------|------------|
| PhaseEstimate with min > max | `rng.triangular(low, high, mode)` raises ValueError if low > high | Validate that `min_minutes <= expected_minutes <= max_minutes` at construction |
| Zero simulation runs | `simulated_totals` is empty; percentile functions return 0.0 | Validate simulation_runs > 0; minimum of 100 for meaningful confidence intervals |
| All multipliers compound to near-zero | A runbook + large team + low severity could produce unrealistically short estimates | Floor multipliers at a minimum (e.g., 0.4) to prevent estimates below physical minimums |
