...
def test_build_summary_core_fields() -> None:
    ...
    payload = project.build_summary(records, "Secure Auth Gateway", "test-run")

    assert payload["project_title"] == "Secure Auth Gateway"
...
def test_main_like_flow_writes_output(tmp_path: Path) -> None:
    ...
    payload = project.build_summary(records, "Secure Auth Gateway", "integration-test")
    ...
    assert loaded["project_title"] == "Secure Auth Gateway"
