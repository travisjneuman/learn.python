"""Tests for Performance Profiler Workbench."""
...
def test_build_summary_core_fields() -> None:
    ...
    payload = project.build_summary(records, "Performance Profiler Workbench", "test-run")

    assert payload["project_title"] == "Performance Profiler Workbench"
...
def test_main_like_flow_writes_output(tmp_path: Path) -> None:
    ...
    payload = project.build_summary(records, "Performance Profiler Workbench", "integration-test")
    ...
    assert loaded["project_title"] == "Performance Profiler Workbench"
