"""Tests for Staff Engineer Capstone."""
...
def test_build_summary_core_fields() -> None:
    ...
    payload = project.build_summary(records, "Staff Engineer Capstone", "test-run")

    assert payload["project_title"] == "Staff Engineer Capstone"
...
def test_main_like_flow_writes_output(tmp_path: Path) -> None:
    ...
    payload = project.build_summary(records, "Staff Engineer Capstone", "integration-test")
    ...
    assert loaded["project_title"] == "Staff Engineer Capstone"
